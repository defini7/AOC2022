...
int main()
{
	...
	while (!file.eof())
	{
		...
		if (
			(pairRanges[0].first <= pairRanges[1].first && pairRanges[0].second >= pairRanges[1].second) ||
			(pairRanges[1].first <= pairRanges[0].first && pairRanges[1].second >= pairRanges[0].second)
		)
			nTotal++;
